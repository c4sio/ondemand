(function ($, vodModule) {
    ...
    // Helpers definition
    function helpers ($log, $window) {
        return {
            /**
             * @description Insert a given separator to the string provided.
             * @param string str The string to handle. The words must be separated with a pipe
             * @param string sep The separator. Defaults to ' - '
             * @return string
             * */
            stringSeparator: function stringSeparator (str, sep) {
                sep = sep || ' - ';
                var arr = [], splitString;
                
                if ( !str ) { throw new Error('stringSeparator: Param "str" is undefined'); }
                if ( str.indexOf('|') === -1  ) { $log.warn('stringSeparator: "|" separator not found on the string provided'); }
                splitString = str.split('|');
                
                $window.angular.forEach(splitString, function (value) {
            ...
            trim: function trim (str) {
                var hasNbsp = str.indexOf('&nbsp;') !== -1,
                    space = ' ';

                if ( hasNbsp ) { space = '&nbsp;'; }

                return str.replace(/^\s+|\s+$/g, space);
            },

            // Performs a search in the given word and replaces a string for another
            stringRemoveSpaces: function removeSpaces (str) {
                return str.replace(/#| /g, '');
            },

            htmlToPlaintext: function htmlToPlaintext(text) {
                return String(text).replace(/<[^>]+>/gm, '');
            },

            /**
             * @description Capitalizes the first letter from a string
            ...
            stringNormalize: function stringNormalize (str) {
            ...
            // 
            numberToJsFormat: function numberToJsFormat (str) {
                var arr, arr2 = [];
    ...
    // Exposing it
    vodModule.service('helpers', ['$log', '$window', helpers]);
})(jQuery, window.angular.module('vod'));